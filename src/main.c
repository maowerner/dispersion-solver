...
int main(int argn, char **argc) {
  ...
  double t, si, sf, ds, eps, eps_b, s_step, s_step_low_high, s, s0, L2, s_min,
      s_max, k;
  double a, b, c, d; // particle masses: a=(m1-m2)**2, b=(m1+m2)**2,
                     // c=(m3-m4)**2, d=(m3+m4)**2
  double *s_cv_plus, *s_cv_minus, *s_below_cut, **ys, *ya,
      *yb;                                               // interpolation grid
  ...
  phase_input(filename, &delta_etapi_spline, &L2_delta_etapi,
              &delta_etapi_const);
  printf("Done.\n\n");
  ...
  N_M_avg[1] = N[3] + N[9]; // 0.5*(METAP^2-MPION^2) to (META+MPION)^2 +
                            // (META+MPION)^2 to (METAP-MPION)^2
  N_M_avg[2] = N[4]; //(METAP-MPION)^2 to (METAP+MPION)^2
  N_M_avg[3] =
      N[5] + N[6]; //(METAP+MPION)^2 to 100*MPION^2 + 100*MPION^2 to s_max

  // number of grid points for Omnes-functions, amplitudes and inhomogeneities
  N_M_int[0] = N_M_avg[0] + N_M_avg[1] + N_M_avg[2] +
               N_M_avg[3];  // upper rim of the cut [s_th,L2]
  ...
  printf("Number of grid points for the Omnes functions, amplitudes and "
         "inhomogeneities:\n");
  for (i = 0; i < 4; i++) {
    printf("N_M_int(%d)=%d\n", i + 1, N_M_int[i]);
  }
  ...
  build_grid(s_cv_plus, s_cv_minus, s_below_cut, s_cmp, ys, s_min, s_max, s0,
             eps, eps_b, a, b, c, d, '0', '+', N, N_sin);
  ...
  printf("Isospin 0...\n");
  build_omnes(omnes0, delta0, s_pipi, L2, delta0_const, s_cv_plus, s_cv_minus,
              s_below_cut, s_cmp, N_M_int);
  printf("Done.\n");
  printf("Isospin 1...\n");
  build_omnes(omnes1, delta1, s_pipi, L2, delta1_const, s_cv_plus, s_cv_minus,
              s_below_cut, s_cmp, N_M_int);
  printf("Done.\n");
  printf("Isospin 2...\n");
  build_omnes(omnes2, delta2, s_pipi, L2, delta2_const, s_cv_plus, s_cv_minus,
              s_below_cut, s_cmp, N_M_int);
  printf("Done.\n\n");
  ...
  fprintf(fout, "%d %+.5e %+.5e %+.5e %+.5e %+.5e %+.5e\n", 0,
          gsl_spline_eval(M0[0].re, s, acc), gsl_spline_eval(M0[0].im, s, acc),
          gsl_spline_eval(M1[0].re, s, acc), gsl_spline_eval(M1[0].im, s, acc),
          gsl_spline_eval(M2[0].re, s, acc), gsl_spline_eval(M2[0].im, s, acc));

  for (i = 1; i < 20; i++) {
    printf("Iteration %d:\n", i);

    printf("Building inhomogenity integrands...\n");
    build_inhomogenity_integrand(M0_hat, M1_hat, M2_hat, F0, G0, F1, G1, F2, G2,
                                 M0_avg, M1_avg, M2_avg, omnes0[0], omnes1[0],
                                 omnes2[0], s_cv_plus, s0, L2, N_M_avg, N_sin,
                                 n0_sub, n1_sub, n2_sub);
    printf("Done.\n\n");

    printf("Computing inhomogenities...\n");
    build_inhomogenity_sqrt(M0_inhom, M0_hat, F0, G0, s0, a, L2, s_cv_plus,
                            s_cv_minus, s_below_cut, s_cmp, N_M_int, n0_sub);
    build_inhomogenity_sqrt_cubed(M1_inhom, M1_hat, F1, G1, s0, a, L2,
                                  s_cv_plus, s_cv_minus, s_below_cut, s_cmp,
                                  N_M_int, n1_sub);
    build_inhomogenity_sqrt(M2_inhom, M2_hat, F2, G2, s0, a, L2, s_cv_plus,
                            s_cv_minus, s_below_cut, s_cmp, N_M_int, n2_sub);
    printf("Done.\n\n");

    printf("Computing Amplitudes...\n");
    build_amplitude(M0, omnes0, M0_inhom, s_cv_plus, s_cv_minus, s_below_cut,
                    s_cmp, ys, s0, L2, N_M_int, n0);
    build_amplitude(M1, omnes1, M1_inhom, s_cv_plus, s_cv_minus, s_below_cut,
                    s_cmp, ys, s0, L2, N_M_int, n1);
    build_amplitude(M2, omnes2, M2_inhom, s_cv_plus, s_cv_minus, s_below_cut,
                    s_cmp, ys, s0, L2, N_M_int, n2);
    printf("Done.\n\n");

    printf("Computing angular averages...\n");
    for (n = 0; n < 2; n++) {
      build_M_avg(M0, M0_avg[n], s_cv_plus, N_M_avg, a, b, c, d, s0, '0', '+',
                  n);
      build_M_avg(M1, M1_avg[n], s_cv_plus, N_M_avg, a, b, c, d, s0, '0', '+',
                  n);
      build_M_avg(M2, M2_avg[n], s_cv_plus, N_M_avg, a, b, c, d, s0, '0', '+',
                  n);
    }
    build_M_avg(M1, M1_avg[n], s_cv_plus, N_M_avg, a, b, c, d, s0, '0', '+', n);
    printf("Done.\n\n");
  ...
  Dispersion_integral_output(filename, M0_inhom, M1_inhom, M2_inhom,
                             s_below_cut, s_cv_plus, N_M_int[2], N_M_int[0]);

  sprintf(filename, "M_%s", sub_const);
  Amplitudes_output(filename, M0, M1, M2, s_below_cut, s_cv_plus, N_M_int[2],
                    N_M_int[0]);
