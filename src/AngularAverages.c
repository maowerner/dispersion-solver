...
void M_avg_1(complex_spline M, complex *M_avg, double *s, double a, double b, double c, double d, char plusminus, char kappa_pm, int N, int n) {
    int i,N_int;
...
//        }
    ...
    for (i=0; i<N; i++) {
        switch (plusminus) {
            ...
            default:
                printf("FATAL ERROR: In function M_avg_1, plusminus has to be '+' or '0' or '-', '%c' is unknown!",plusminus);
                exit(1);
                break;
        }
        ...
        switch (kappa_pm) {
            ...
            default:
                printf("FATAL ERROR: In function M_avg_1, kappa_pm has to be '+' or '-', '%c' is unknown!",kappa_pm);
                break;
        }
...
void M_avg_2(complex_spline M_plus, complex_spline M_minus, complex *M_avg, double *s, double a, double b, double c, double d, double s0, char plusminus, char kappa_pm, int N, int n) {
    int i,N_int;
    double eps,error,s_minus,s_plus,sigma;
    complex temp;
    ...
    double f_re_minus(double x, void * params) {
        ...
        if (isnan(f)==1) {
            printf("%d %.10e %.10e %.10e\n",n,x,s0,gsl_spline_eval(M_minus.re,x,acc_minus));
        }
    ...
    double f_im_minus(double x, void * params) {
        ...
        if (isnan(f)==1) {
            printf("%d %.10e %.10e %.10e\n",n,x,s0,gsl_spline_eval(M_minus.im,x,acc_minus));
        }
        return f;
    }
    ...
    for (i=0; i<N; i++) {
        ...
        F.params = &sigma;
        gsl_integration_qags(&F,s0+1.e-10,s_plus,eps,eps,N_int,w,&temp.re,&error);
        
        G.params = &sigma;
        gsl_integration_qags(&G,s0+1.e-10,s_plus,eps,eps,N_int,w,&temp.im,&error);
        
        M_avg[i].re = temp.re;
        M_avg[i].im = temp.im;
        
        H.params = &sigma;
        gsl_integration_qags(&H,s_minus,s0+1.e-10,eps,eps,N_int,w,&temp.re,&error);
        
        I.params = &sigma;
        gsl_integration_qags(&I,s_minus,s0+1.e-10,eps,eps,N_int,w,&temp.im,&error);
...
complex gamma_param(double s) {
    ...
    gamma.im = 0.5*sqrt(1.-4./s)*sqrt((METAP_M_MPION_SQUARED-s)*(s-METAP_P_MPION_SQUARED));
...
complex dgamma_param(double s) {
    ...
    dgamma.re = -0.5;
    dgamma.im = (4.*METAP_M_MPION_SQUARED*METAP_P_MPION_SQUARED-(4.+METAP_M_MPION_SQUARED+METAP_P_MPION_SQUARED)*s*s+2.*s*s*s)/(4.*sqrt((METAP_M_MPION_SQUARED-s)*(s-METAP_P_MPION_SQUARED))*sqrt(s-4.)*pow(s,1.5));
    
    return dgamma;
}

void M_avg_III(complex_spline *M, complex *M_avg, double *s, double a, double b, double c, double d, char plusminus, char kappa_pm, int N, double n) {
    int i,N_int;
    double eps,error,sigma,temp,s_ab_avg,yb,yab_avg,yba_avg,yi,y,r,phi,c1,s1,c2,s2,f;
    ...
    yb = pow(b,1./4.);
    yab_avg = sqrt(0.5*(sqrt(s_ab_avg)+sqrt(a)))-sqrt(0.5*(sqrt(s_ab_avg)-sqrt(a)));
    yba_avg = sqrt(0.5*(sqrt(b)+sqrt(s_ab_avg)))-sqrt(0.5*(sqrt(b)-sqrt(s_ab_avg)));
    ...
    double fa_re(double x, void * params) {
        ...
        M_sub.re = c1*gsl_spline_eval(M[0].re,x,acc_a)-c2*gsl_spline_eval(M[2].re,x,acc_a);
        M_sub.im = s1*gsl_spline_eval(M[0].im,x,acc_a)-s2*gsl_spline_eval(M[2].im,x,acc_a);
        
        M_add.re = s1*gsl_spline_eval(M[0].re,x,acc_a)+s2*gsl_spline_eval(M[2].re,x,acc_a);
        M_add.im = c1*gsl_spline_eval(M[0].im,x,acc_a)+c2*gsl_spline_eval(M[2].im,x,acc_a);
        
        f = pow(r,n)*(dgamma.re*M_sub.re+dgamma.im*M_add.im-dgamma.re*M_sub.im+dgamma.im*M_add.re);
        
        if (isnan(f)==1) {
            printf("fa_re: %.6e %.6e %.6e %.6e %.6e\n",x,dgamma.re,dgamma.im,M_sub.re,M_sub.im);
        }
    ...
    double fb_re(double x, void * params) {
        ...
        M_sub.re = c1*gsl_spline_eval(M[1].re,x,acc_b)-c2*gsl_spline_eval(M[3].re,x,acc_b);
        M_sub.im = s1*gsl_spline_eval(M[1].im,x,acc_b)-s2*gsl_spline_eval(M[3].im,x,acc_b);
        
        M_add.re = s1*gsl_spline_eval(M[1].re,x,acc_b)+s2*gsl_spline_eval(M[3].re,x,acc_b);
        M_add.im = c1*gsl_spline_eval(M[1].im,x,acc_b)+c2*gsl_spline_eval(M[3].im,x,acc_b);
    ...
    double fa_im(double x, void * params) {
        ...
        M_sub.re = s1*gsl_spline_eval(M[0].re,x,acc_a)-s2*gsl_spline_eval(M[2].re,x,acc_a);
        M_sub.im = c1*gsl_spline_eval(M[0].im,x,acc_a)-c2*gsl_spline_eval(M[2].im,x,acc_a);
        
        M_add.re = c1*gsl_spline_eval(M[0].re,x,acc_a)+c2*gsl_spline_eval(M[2].re,x,acc_a);
        M_add.im = s1*gsl_spline_eval(M[0].im,x,acc_a)+s2*gsl_spline_eval(M[2].im,x,acc_a);
        
        f = pow(r,n)*(dgamma.re*M_sub.im-dgamma.im*M_add.re+dgamma.re*M_sub.re+dgamma.im*M_add.im);
    ...
    double fb_im(double x, void * params) {
        ...
        M_sub.re = s1*gsl_spline_eval(M[1].re,x,acc_b)-s2*gsl_spline_eval(M[3].re,x,acc_b);
        M_sub.im = c1*gsl_spline_eval(M[1].im,x,acc_b)-c2*gsl_spline_eval(M[3].im,x,acc_b);
        
        M_add.re = c1*gsl_spline_eval(M[1].re,x,acc_b)+c2*gsl_spline_eval(M[3].re,x,acc_b);
        M_add.im = s1*gsl_spline_eval(M[1].im,x,acc_b)+s2*gsl_spline_eval(M[3].im,x,acc_b);
    ...
    for (i=0; i<N; i++) {
        switch (plusminus) {
            ...
            default:
                printf("FATAL ERROR: In function M_avg_III, plusminus has to be '+' or '0' or '-', '%c' is unknown!",plusminus);
                exit(1);
                break;
        }
        ...
        if (s[i]>s_ab_avg) {
            ...
            gsl_integration_qags(&Fb_re,yi,yb,eps,eps,N_int,w,&M_avg[i].re,&error);
            gsl_integration_qags(&Fb_im,yi,yb,eps,eps,N_int,w,&M_avg[i].im,&error);
        }
        
        else {
            yi = s_to_y(s[i],a,b);
            
            gsl_integration_qags(&Fb_re,yba_avg,yb,eps,eps,N_int,w,&M_avg[i].re,&error);
            gsl_integration_qags(&Fb_im,yba_avg,yb,eps,eps,N_int,w,&M_avg[i].im,&error);
            
            gsl_integration_qags(&Fa_re,yi,yab_avg,eps,eps,N_int,w,&temp,&error);
            M_avg[i].re += temp;
            gsl_integration_qags(&Fa_im,yi,yab_avg,eps,eps,N_int,w,&temp,&error);
            M_avg[i].im += temp;
        }
...
void M_avg_4(complex_spline M, complex *M_avg, double *s, double a, double b, double c, double d, char plusminus, char kappa_pm, int N, int n) {
    int i,N_int;
...
void build_M_avg(complex_spline *M, complex *M_avg, double *s, int *N, double a, double b, double c, double d, double s0, char plusminus, char kappa_pm, int n) {
    M_avg_1(M[0],M_avg,s,a,b,c,d,plusminus,kappa_pm,N[0],n);
    M_avg_2(M[0],M[1],M_avg+N[0],s+N[0],a,b,c,d,s0,plusminus,kappa_pm,N[1],n);
    M_avg_III(M+3,M_avg+N[0]+N[1],s+N[0]+N[1],a,b,c,d,plusminus,kappa_pm,N[2],n);
    M_avg_4(M[2],M_avg+N[0]+N[1]+N[2],s+N[0]+N[1]+N[2],a,b,c,d,plusminus,kappa_pm,N[3],n);
}

//
void build_inhomogenity_integrand(complex_spline *M0, complex_spline *M1, complex_spline *M2, complex *cf0, complex *cg0, complex *cf1, complex *cg1, complex *cf2, complex *cg2, complex **M00, complex **M11, complex **M20, complex *omnes0, complex *omnes1, complex *omnes2, double *s, double s0, double L2, int *N, int Nsin, int n0, int n1, int n2) {
    int i,j,n,m,N1,N2;
    ...
    for (j=0; j<4; j++) {
        ...
        for (i=0; i<N[j]; i++) {
            ...
            c0 = sin(delta0(s[i+n]))/(pow(s[i+n],(double)n0)*k*sqrt(omnes0[i+n].re*omnes0[i+n].re+omnes0[i+n].im*omnes0[i+n].im));
            c1 = sin(delta1(s[i+n]))/(pow(s[i+n],(double)n1)*k*k*k*sqrt(omnes1[i+n].re*omnes1[i+n].re+omnes1[i+n].im*omnes1[i+n].im));
            c2 = sin(delta2(s[i+n]))/(pow(s[i+n],(double)n2)*k*sqrt(omnes2[i+n].re*omnes2[i+n].re+omnes2[i+n].im*omnes2[i+n].im));
...
void build_inhomogenity_integrand_etap_eta2pi(complex_spline *M0, complex_spline *M1, complex *cf0, complex *cg0, complex *cf1, complex *cg1, complex *M0_avg, complex *M1_avg_s, complex *M1_avg_u, complex *omnes0, complex *omnes1, double *s, double *t, double s_th, double s_ps, double a_s, double b_s, double t_th, double t_ps, double a_t, double b_t, double L2, int *N_s, int *N_t, int Nsin, int n0, int n1) {
    int i,j,n,m,N1,N2;
    double si,sf,s_am,s_ap,k,c,*M0_re,*M0_im,*M1_re,*M1_im,s_f[4],s_g[4],abs_omnes;
    complex f0[4],g0[4],f1[4],g1[4];
    
    M0_re = (double *)malloc((N_s[0]+N_s[1]+N_s[2]+N_s[3])*sizeof(double));
    M0_im = (double *)malloc((N_s[0]+N_s[1]+N_s[2]+N_s[3])*sizeof(double));
    
    M1_re = (double *)malloc((N_t[0]+N_t[1]+N_t[2]+N_t[3])*sizeof(double));
    M1_im = (double *)malloc((N_t[0]+N_t[1]+N_t[2]+N_t[3])*sizeof(double));
    ...
    for (j=0; j<4; j++) {
        ...
        for (i=0; i<N_s[j]; i++) {
            k = sqrt(fabs((s[i+n]-s_ps)*(s[i+n]-s_th)))*sqrt(fabs(s[i+n]-b_s))/s[i+n];
            abs_omnes = sqrt(omnes0[i+n].re*omnes0[i+n].re+omnes0[i+n].im*omnes0[i+n].im);
    ...
    for (j=0; j<4; j++) {
        ...
        for (i=0; i<N_t[j]; i++) {
            k = sqrt(fabs((t[i+n]-t_ps)*(t[i+n]-t_th)))*sqrt(fabs(t[i+n]-b_t))/t[i+n];
            abs_omnes = sqrt(omnes1[i+n].re*omnes1[i+n].re+omnes1[i+n].im*omnes1[i+n].im);
            c = sin(delta_etapi(t[i+n]))/(pow(t[i+n],(double)n1)*k*abs_omnes);
...
//    printf("g(x)=%+.5e%+.5e*sqrt(x-%.5e)%+.5e*(x-%.5e)%+.5e*(x-%.5e)**1.5\n",cg1[0].re,cg1[1].re,a_t,cg1[2].re,a_t,cg1[3].re,a_t);
    ...
    gsl_spline_init(M0[1].re,s+N_s[0]+N_s[1],M0_re+N_s[0]+N_s[1],N_s[2]+N_s[3]);
    gsl_spline_init(M0[1].im,s+N_s[0]+N_s[1],M0_im+N_s[0]+N_s[1],N_s[2]+N_s[3]);
    ...
    gsl_spline_init(M1[1].re,t+N_t[0]+N_t[1],M1_re+N_t[0]+N_t[1],N_t[2]+N_t[3]);
    gsl_spline_init(M1[1].im,t+N_t[0]+N_t[1],M1_im+N_t[0]+N_t[1],N_t[2]+N_t[3]);
