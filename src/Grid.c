...
void grid_point_size(int *N, int Nsin, double s_step, double s_step_low_high,
  ...
  if (METAP != META) {
    ...
    N[3] = (int)ceil((METAP_M_MPION_SQUARED - 0.5 * (METAP_SQUARED - 1.0)) /
                     s_step) +
           Nsin;
    N[9] = 0;
  }
  ...
  N[4] = (int)ceil(((METAP_P_MPION_SQUARED - eps_b) - METAP_M_MPION_SQUARED) /
                   s_step) +
         Nsin;
...
void Ngrid_etap_eta2pi(int *N, int Nsin, double s_step, double s_step_low_high,
  ...
  N[2] = (int)ceil((4. - 4. * pow(MPION0 / MPION, 2.)) / (0.1 * s_step)) +
         2 * Nsin;
  ...
  N[16] = (int)ceil(M_PI * fabs(cmp_im(gamma_path(0.5 * (a_t + b_t), a_t, b_t,
                                                  t_ps, t_th, '-', '-'))) /
                    (2. * s_step));

  // complex plane for t-channel tp-variable
  N[17] = (int)ceil(M_PI * fabs(cmp_im(gamma_path(0.5 * (a_s + b_s), a_s, b_s,
                                                  s_ps, s_th, '0', '-'))) /
                    (2. * s_step));

  // complex plane for t-channel up-variable
  N[18] = (int)ceil(M_PI * fabs(cmp_im(gamma_path(0.5 * (a_t + b_t), a_t, b_t,
                                                  t_ps, t_th, '+', '-'))) /
                    (2. * s_step));
  ;
}
...
// build interval of the form [si,sf[ where Nsin points approach sf by
// (1/2)^Nsin the distance to sf
...
// build interval of the form ]si,sf] where Nsin points approach si by
// (1/2)^Nsin the distance to si
...
// build interval of the form ]si,sf[ where Nsin points approach si/sf by
// (1/2)^Nsin the distance to si/sf
...
void grid_etap_eta2pi(double **s, double **ys, double **yt, double **yu,
  ...
  // complex plane for s-channel sp-variable
  // ya
  y_min = 0.5 * (sqrt(sqrt(2. * (a_t + b_t)) + 2. * sqrt(a_t)) -
                 sqrt(sqrt(2. * (a_t + b_t)) - 2. * sqrt(a_t)));
  y_max = pow(a_t, 1. / 4.);
  closed_to_closed(ys[0], y_min, y_max, N[16]);
  // yb
  y_min = 0.5 * (sqrt(2. * sqrt(b_t) + sqrt(2. * (a_t + b_t))) -
                 sqrt(2. * sqrt(b_t) - sqrt(2. * (a_t + b_t))));
  y_max = pow(b_t, 1. / 4.);
  ...
  // complex plane for t-channel tp-variable
  // ya
  y_min = 0.5 * (sqrt(sqrt(2. * (a_s + b_s)) + 2. * sqrt(a_s)) -
                 sqrt(sqrt(2. * (a_s + b_s)) - 2. * sqrt(a_s)));
  y_max = pow(a_s, 1. / 4.);
  closed_to_closed(yt[0], y_min, y_max, N[17]);
  // yb
  y_min = 0.5 * (sqrt(2. * sqrt(b_s) + sqrt(2. * (a_s + b_s))) -
                 sqrt(2. * sqrt(b_s) - sqrt(2. * (a_s + b_s))));
  y_max = pow(b_s, 1. / 4.);
...
void build_s_below_cut(double *s, double s_min, double s_th, double eps_a,
                       int *N, int Nsin) {
  int i, n, m;
  double si, sf, ds;
...
void build_s_cv_plus(double *s, double s_th, double s_max, double eps_a,
                     double eps_b, int *N, int Nsin) {
  int i, n, m;
  double si, sf, ds;
...
void build_s_complex(complex **s, double **y, double a, double b, double c,
                     double d, char plusminus, char kappa_pm, int *N) {
  int i, n;
  ...
  ya_min = 0.5 * (sqrt(sqrt(2. * (a + b)) + 2. * sqrt(a)) -
                  sqrt(sqrt(2. * (a + b)) - 2. * sqrt(a)));
  ya_max = pow(a, 1. / 4.);
  dya = (ya_max - ya_min) / (n - 1);

  yb_min = 0.5 * (sqrt(2. * sqrt(b) + sqrt(2. * (a + b))) -
                  sqrt(2. * sqrt(b) - sqrt(2. * (a + b))));
  yb_max = pow(b, 1. / 4.);
...
void build_grid(double *s_cv_plus, double *s_cv_minus, double *s_below_cut,
  ...
  for (i = 0; i < n; i++) {
    if (s_below_cut[i] >= s_below_cut[i + 1] && i + 1 < n - 1) {
      printf("FATAL ERROR: s_below_cut is not stictly monotonically "
             "increasing! step=%d, s=%.10e\n\n",
             i, s_below_cut[i]);
      exit(1);
    }
  }
  ...
  for (i = 0; i < n; i++) {
    if (s_cv_plus[i] >= s_cv_plus[i + 1] && i + 1 < n - 1) {
      printf("FATAL ERROR: s_cv_plus is not stictly monotonically increasing! "
             "step=%d, s=%.10e\n\n",
             i, s_cv_plus[i]);
      exit(1);
    }
  ...
  for (i = 0; i < n; i++) {
    if (s_cv_minus[i] >= s_cv_minus[i + 1] && i + 1 < n - 1) {
      printf("FATAL ERROR: s_cv_minus is not stictly monotonically increasing! "
             "step=%d, s=%.10e\n\n",
             i, s_cv_minus[i]);
      exit(1);
    }
  ...
  for (i = 0; i < n; i++) {
    if (y[0][i] >= y[0][i + 1] && i + 1 < n - 1) {
      printf("FATAL ERROR: ya is not stictly monotonically increasing! "
             "step=%d, ya=%.10e\n\n",
             i, y[0][i]);
      exit(1);
    }
    if (y[1][i] >= y[1][i + 1] && i + 1 < n - 1) {
      printf("FATAL ERROR: yb is not stictly monotonically increasing! "
             "step=%d, yb=%.10e\n\n",
             i, y[1][i]);
      exit(1);
    }
