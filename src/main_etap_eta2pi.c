...
double test_func(double s) { return delta0(s) + delta_etapi(s); }

void build_M_avg_u(complex_spline *M, complex *M_avg, double *s, int *N,
                   double a, double b, double c, double d, double s0,
                   char plusminus, char kappa_pm, int n) {
  M_avg_1(M[0], M_avg, s, a, b, c, d, plusminus, kappa_pm, N[0], n);
  M_avg_2(M[0], M[1], M_avg + N[0], s + N[0], a, b, c, d, s0, plusminus,
          kappa_pm, N[1], n);
  M_avg_III(M + 7, M_avg + N[0] + N[1], s + N[0] + N[1], a, b, c, d, plusminus,
            kappa_pm, N[2], n);
...
double convergence_check(complex *Mi, complex *Mj, int N) {
  ...
  for (i = 0; i < N; i++) {
    sum += sqrt(
        (pow(Mi[i].re - Mj[i].re, 2.) + pow(Mi[i].im - Mj[i].im, 2.) + 1.e-10) /
        (Mi[i].re * Mi[i].re + Mi[i].im * Mi[i].im + 1.e-10));
  }
...
int main(int argn, char **argc) {
  ...
  int N_amp_s[4], N_avg_s[4], N_amp_t[4], N_avg_t[4], N_amp_u, N_avg_u[4],
      N_hat_s[2], N_hat_t[2];
  double t, eps, eps_b, s_step, s_step_low_high, s, s0, L2, s_min, s_max,
      s_minus, t_minus, k, sum;
  double m1, m2, m3, m4; // particle masses
  double a, b, c, d;
  double a_s, b_s, s_ps, s_th, s_m, s_12, a_t, b_t, t_ps, t_th, t_m, t_12, a_u,
      b_u, u_ps, u_th, u_m, u_12;
  double *s_cv_plus, *s_cv_minus, *s_below_cut, *phi, *ya,
      *yb;                                      // interpolation grid s-channel
  ...
  if (s_th > a_s) {
    printf(
        "FATAL ERROR: particle masses have been set wrongly, m1<=m2<=m3<m4!\n");
    exit(1);
  }
  // printf("%.3f %.3f %.3f %.3f %.3f\n",s_th,a_s,b_s,s_m,s_12);

  // t-channel variables
  /*! @todo What are the variables t_th, t_ps, a_t, b_t, t_m, t_12? */
  t_th = pow(m1 + m3, 2.);
  t_ps = pow(m1 - m3, 2.);
  a_t = pow(m2 - m4, 2.);
  b_t = pow(m2 + m4, 2.);
  ...
  /****************************************************************************/
  /*! 1. Read phaseshifts from ./Input and interpolate values with gsl spline 
   */
  ...
  /*! file_delta0 declared in Basic.h and set in Basic.h
   *  delta_high_energy_tale declared and defined in PhaseShifts
   *  phase_input in declared and defined in InputOutput
   */
  ...
  /*! 2. Set up grid (Grid for what?)
   */

  // calculating number of grid points
  /* Ngrid_etap_eta2pi declared and defined in Grid 
   */
  Ngrid_etap_eta2pi(N, N_sin, s_step, s_step_low_high, s_min, s_max, s_th, a_s,
                    b_s, s_ps, s_m, s_12, t_th, a_t, b_t, t_ps, t_m, t_12, u_m,
                    u_12, eps_b);
  ...
  N_avg_s[2] =
      N[5] + N[6] + N[7] + N[8] + N[9] + N[10] + N[11] + N[12]; //[a_s,b_s]
  ...
  N_amp_s[0] = N_avg_s[0] + N_avg_s[1] + N_avg_s[2] +
               N_avg_s[3]; // upper rim of the cut [s_th,s_max]
  ...
  N_amp_t[0] = N_avg_t[0] + N_avg_t[1] + N_avg_t[2] +
               N_avg_t[3]; // upper rim of the cut [t_th,s_max]
  N_amp_t[1] = N[15];      // lower rim of the cut [t_th,s_m]
  N_amp_t[2] =
      N[0] + N[1] + N[2] + N[3] + N[4] + N[5]; // left from the cut [s_min,t_th]
  ...
  grid_etap_eta2pi(s_grid, ys, yt, yu, s_cmp, t_cmp, u_cmp, N, N_sin, s_step,
                   s_step_low_high, s_min, s_max, s_th, a_s, b_s, s_ps, s_m,
                   s_12, t_th, a_t, b_t, t_ps, t_m, t_12, u_m, u_12, eps_b,
                   eps);

  build_grid_etap_eta2pi(s_grid, N, s_below_cut, s_cv_plus, s_cv_minus,
                         t_below_cut, t_cv_plus, t_cv_minus, pi0pi0);

  /*! 3. Build omnes_I functions (What is that) 
   *  Memory is allocated for omnes_I functions, inhomogeneity integrals, 
   *  amplitudes, angular averages, angular average convergence check and hat function.
   *  @todo Encapsulate memory allocation and completely encapsulate everything that 
   *  is not needed lateron anymore (find out what that is)
   */
  ...
  complex *M1_avg_s_convergence =
      (complex *)malloc(N_amp_t[0] * sizeof(complex));
  complex *M1_avg_u_convergence =
      (complex *)malloc(N_amp_t[0] * sizeof(complex));
  ...
  /*! 4. Build the Omnes function and its derivatives and write to file
  /*! build_omnes and omnes_complex declared in Omnes 
   */
  printf("Computing Omnes functions...\n");
  printf("Isospin 0...\n");
  build_omnes(omnes0, delta0, delta0_params, s_cv_plus, s_cv_minus, s_below_cut,
              s_cmp, N_amp_s);
  printf("Done.\n");
  printf("Isospin 1...\n");
  build_omnes(omnes1, delta_etapi, delta_etapi_params, t_cv_plus, t_cv_minus,
              t_below_cut, t_cmp, N_amp_t);
  ...
  /*! dOmnes0 and d2Omnes0 are declared in Omnes. What are they doing? 
   *  Name suggest calculation of derivative, but internally calls 
  ...
  for (i = 0; i < N_amp_s[0]; i++) {
    if (s_cv_plus[i] < t_th) {
      fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
              omnes0[0][i].re, omnes0[0][i].im, omnes1[2][i + N_amp_s[2]].re,
              omnes1[2][i + N_amp_s[2]].im);
    } else {
      fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
              omnes0[0][i].re, omnes0[0][i].im, omnes1[0][j].re,
              omnes1[0][j].im);
      j++;
    }
  }
  ...
  for (I = 0; I < N_SUB_CONST; I++) {
    ...
    build_amplitude(M0, omnes0, M0_inhom, s_cv_plus, s_cv_minus, s_below_cut,
                    s_cmp, ys, s_th, L2, N_amp_s, n0);
    build_amplitude(M1, omnes1, M1_inhom, t_cv_plus, t_cv_minus, t_below_cut,
                    t_cmp, yt, t_th, L2, N_amp_t, n1);
    amplitude_complex(M1[7], omnes1[5], M1_inhom[7], u_cmp[0], yu[0], '-',
                      N_amp_u, n1);
    amplitude_complex(M1[8], omnes1[6], M1_inhom[8], u_cmp[1], yu[1], '-',
                      N_amp_u, n1);
    amplitude_complex(M1[9], omnes1[5], M1_inhom[9], u_cmp[0], yu[0], '+',
                      N_amp_u, n1);
    amplitude_complex(M1[10], omnes1[6], M1_inhom[10], u_cmp[1], yu[1], '+',
                      N_amp_u, n1);
    printf("Done.\n\n");

    printf("Computing angular averages...\n");
    build_M_avg(M1, M0_avg, s_cv_plus, N_avg_s, a_s, b_s, s_ps, s_th, t_th, '0',
                '+', 0);
    build_M_avg(M0, M1_avg_s, t_cv_plus, N_avg_t, a_t, b_t, t_ps, t_th, s_th,
                '-', '-', 0);
    build_M_avg_u(M1, M1_avg_u, t_cv_plus, N_avg_u, a_t, b_t, t_ps, t_th, t_th,
                  '+', '+', 0);
    printf("Done.\n\n");
    ...
    sum = (convergence_check(M0_avg, M0_avg_convergence, N_amp_s[0]) +
           convergence_check(M1_avg_s, M1_avg_s_convergence, N_amp_t[0]) +
           convergence_check(M1_avg_u, M1_avg_u_convergence, N_amp_t[0])) /
          3.;
    ...
    fprintf(fout, "%d %+.5e %+.5e %+.5e %+.5e %+.5e\n", 0,
            gsl_spline_eval(M0[0].re, s, acc0),
            gsl_spline_eval(M0[0].im, s, acc0),
            gsl_spline_eval(M1[0].re, s, acc1),
            gsl_spline_eval(M1[0].im, s, acc1), sum);

    for (i = 1; i < N_ITER; i++) {
      printf("Iteration %d:\n", i);

      printf("Building inhomogenity integrands...\n");
      build_inhomogenity_integrand_etap_eta2pi(
          M0_hat, M1_hat, F0, G0, F1, G1, M0_avg, M1_avg_s, M1_avg_u, omnes0[0],
          omnes1[0], s_cv_plus, t_cv_plus, s_th, s_ps, a_s, b_s, t_th, t_ps,
          a_t, b_t, L2, N_avg_s, N_avg_t, N_sin, n0_sub, n1_sub);
      printf("Done.\n\n");

      printf("Computing inhomogenities...\n");
      build_inhomogenity_sqrt(M0_inhom, M0_hat, F0, G0, s_th, a_s, L2,
                              s_cv_plus, s_cv_minus, s_below_cut, s_cmp,
                              N_amp_s, n0_sub);
      build_inhomogenity_sqrt(M1_inhom, M1_hat, F1, G1, t_th, a_t, L2,
                              t_cv_plus, t_cv_minus, t_below_cut, t_cmp,
                              N_amp_t, n1_sub);
      inhomogenity_sqrt_complex(M1_inhom[7], M1_hat, F1, G1, u_cmp[0], '-',
                                t_th, a_t, L2, N_amp_u, n1_sub);
      inhomogenity_sqrt_complex(M1_inhom[8], M1_hat, F1, G1, u_cmp[1], '-',
                                t_th, a_t, L2, N_amp_u, n1_sub);
      inhomogenity_sqrt_complex(M1_inhom[9], M1_hat, F1, G1, u_cmp[0], '+',
                                t_th, a_t, L2, N_amp_u, n1_sub);
      inhomogenity_sqrt_complex(M1_inhom[10], M1_hat, F1, G1, u_cmp[1], '+',
                                t_th, a_t, L2, N_amp_u, n1_sub);
      printf("Done.\n\n");
      ...
      printf("Computing angular averages...\n");
      build_M_avg(M1, M0_avg, s_cv_plus, N_avg_s, a_s, b_s, s_ps, s_th, t_th,
                  '0', '+', 0);
      build_M_avg(M0, M1_avg_s, t_cv_plus, N_avg_t, a_t, b_t, t_ps, t_th, s_th,
                  '-', '-', 0);
      build_M_avg_u(M1, M1_avg_u, t_cv_plus, N_avg_u, a_t, b_t, t_ps, t_th,
                    t_th, '+', '+', 0);
      printf("Done.\n\n");
    ...
    for (i = 0; i < N_amp_s[0]; i++) {
      if (s_cv_plus[i] < t_th) {
        fprintf(fout, "%.10e %.10e %.10e\n", s_cv_plus[i], M0_avg[i].re,
                M0_avg[i].im);
      } else {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e %.10e %.10e\n",
                s_cv_plus[i], M0_avg[i].re, M0_avg[i].im, M1_avg_s[j].re,
                M1_avg_s[j].im, M1_avg_u[j].re, M1_avg_u[j].im);
        j++;
      }
    }
    fclose(fout);
    ...
    for (i = 0; i < N_amp_s[2]; i++) {
      fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_below_cut[i],
              gsl_spline_eval(M0[2].re, s_below_cut[i], acc_s0),
              gsl_spline_eval(M0[2].im, s_below_cut[i], acc_s0),
              gsl_spline_eval(M1[2].re, s_below_cut[i], acc_t0),
              gsl_spline_eval(M1[2].im, s_below_cut[i], acc_t0));
    }
    for (i = 0; i < N_amp_s[0]; i++) {
      if (s_cv_plus[i] < t_th) {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                gsl_spline_eval(M0[0].re, s_cv_plus[i], acc_s1),
                gsl_spline_eval(M0[0].im, s_cv_plus[i], acc_s1),
                gsl_spline_eval(M1[2].re, s_cv_plus[i], acc_t0),
                gsl_spline_eval(M1[2].im, s_cv_plus[i], acc_t0));
      } else {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                gsl_spline_eval(M0[0].re, s_cv_plus[i], acc_s1),
                gsl_spline_eval(M0[0].im, s_cv_plus[i], acc_s1),
                gsl_spline_eval(M1[0].re, s_cv_plus[i], acc_t1),
                gsl_spline_eval(M1[0].im, s_cv_plus[i], acc_t1));
      }
    }
    fclose(fout);
    ...
    for (i = 0; i < N_amp_s[0]; i++) {
      if (s_cv_plus[i] < t_th && s_cv_plus[i] < a_s) {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                gsl_spline_eval(M0_hat[0].re, s_cv_plus[i], acc_s2),
                gsl_spline_eval(M0_hat[0].im, s_cv_plus[i], acc_s2), 0., 0.);
      } else if (s_cv_plus[i] < t_th && s_cv_plus[i] > a_s) {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                gsl_spline_eval(M0_hat[1].re, s_cv_plus[i], acc_s3),
                gsl_spline_eval(M0_hat[1].im, s_cv_plus[i], acc_s3), 0., 0.);
      } else if (s_cv_plus[i] > t_th && s_cv_plus[i] < a_t) {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                gsl_spline_eval(M0_hat[1].re, s_cv_plus[i], acc_s3),
                gsl_spline_eval(M0_hat[1].im, s_cv_plus[i], acc_s3),
                gsl_spline_eval(M1_hat[0].re, s_cv_plus[i], acc_t2),
                gsl_spline_eval(M1_hat[0].im, s_cv_plus[i], acc_t2));
      } else if (s_cv_plus[i] > t_th && s_cv_plus[i] > a_t) {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                gsl_spline_eval(M0_hat[1].re, s_cv_plus[i], acc_s3),
                gsl_spline_eval(M0_hat[1].im, s_cv_plus[i], acc_s3),
                gsl_spline_eval(M1_hat[1].re, s_cv_plus[i], acc_t3),
                gsl_spline_eval(M1_hat[1].im, s_cv_plus[i], acc_t3));
      }
    }
    fclose(fout);
    ...
    for (i = 0; i < N_amp_s[0]; i++) {
      if (s_cv_plus[i] < t_th) {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                M0_inhom[0][i].re, M0_inhom[0][i].im,
                M1_inhom[2][i + N_amp_s[2]].re, M1_inhom[2][i + N_amp_s[2]].im);
      } else {
        fprintf(fout, "%.10e %.10e %.10e %.10e %.10e\n", s_cv_plus[i],
                M0_inhom[0][i].re, M0_inhom[0][i].im, M1_inhom[0][j].re,
                M1_inhom[0][j].im);
        j++;
      }
    }
    ...
    test_tmp = inhomogenity_sqrt_below_cut_single_value(M0_hat, F0, G0, 0.,
                                                        s_th, a_s, b_s, n0_sub);
    printf("I0_%s = %+.6e %+.6e i\n", SUB_CONST[I], test_tmp.re, test_tmp.im);

    test_tmp = inhomogenity_sqrt_below_cut_single_value(M1_hat, F1, G1, 0.,
                                                        t_th, a_t, b_t, n0_sub);
