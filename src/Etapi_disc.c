...
void build_inhomogenity_integrand_etapi_disc(
    complex_spline M0_finite, complex_spline M2_finite, complex_spline *M0_sing,
    complex_spline *M2_sing, complex *cf0, complex *cg0, complex *cf2,
    complex *cg2, complex_spline f0etapi, complex_spline f2etapi,
    complex_spline Metapi, complex_spline *Metapi_tilde, gsl_spline *delta0,
    gsl_spline *delta2, double *abs_omnes0, double *abs_omnes2, double *s,
    double s0, double L2, int *N, int Nsin, int n0, int n2) {
  int i, j, n, m, N1, N2;
  double si, sf, s_am, s_ap, l, d0, d2, c0, c2, *M0_re, *M0_im, *M2_re, *M2_im,
      *M0_tilde_re, *M0_tilde_im, *M2_tilde_re, *M2_tilde_im, s_f[5], s_g[5];
  ...
  for (i = 0; i < N[0]; i++) {
    ...
    temp0.re = gsl_spline_eval(f0etapi.re, s[i], acc_f0) *
                   gsl_spline_eval(Metapi.re, s[i], acc_M) +
               gsl_spline_eval(f0etapi.im, s[i], acc_f0) *
                   gsl_spline_eval(Metapi.im, s[i], acc_M);
    temp0.im = gsl_spline_eval(f0etapi.re, s[i], acc_f0) *
                   gsl_spline_eval(Metapi.im, s[i], acc_M) -
               gsl_spline_eval(f0etapi.im, s[i], acc_f0) *
                   gsl_spline_eval(Metapi.re, s[i], acc_M);
    temp2.re = gsl_spline_eval(f2etapi.re, s[i], acc_f2) *
                   gsl_spline_eval(Metapi.re, s[i], acc_M) +
               gsl_spline_eval(f2etapi.im, s[i], acc_f2) *
                   gsl_spline_eval(Metapi.im, s[i], acc_M);
    temp2.im = gsl_spline_eval(f2etapi.re, s[i], acc_f2) *
                   gsl_spline_eval(Metapi.im, s[i], acc_M) -
               gsl_spline_eval(f2etapi.im, s[i], acc_f2) *
                   gsl_spline_eval(Metapi.re, s[i], acc_M);
    ...
    temp0.re = gsl_spline_eval(f0etapi.re, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[0].re, s[i], acc_M_tilde_low) +
               gsl_spline_eval(f0etapi.im, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[0].im, s[i], acc_M_tilde_low);
    temp0.im = gsl_spline_eval(f0etapi.re, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[0].im, s[i], acc_M_tilde_low) -
               gsl_spline_eval(f0etapi.im, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[0].re, s[i], acc_M_tilde_low);
  ...
  for (i = N[0]; i < n; i++) {
    ...
    temp0.re = gsl_spline_eval(f0etapi.re, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[1].re, s[i], acc_M_tilde_high) +
               gsl_spline_eval(f0etapi.im, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[1].im, s[i], acc_M_tilde_high);
    temp0.im = gsl_spline_eval(f0etapi.re, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[1].im, s[i], acc_M_tilde_high) -
               gsl_spline_eval(f0etapi.im, s[i], acc_f0) *
                   gsl_spline_eval(Metapi_tilde[1].re, s[i], acc_M_tilde_high);
  ...
  for (i = (N[0] - Nsin); i < N[0]; i++) {
    ...
    M0_tilde_re[i] = cf0[0].re + cf0[1].re * si + cf0[2].re * si * si +
                     cf0[3].re * si * si * si;
    M0_tilde_im[i] = cf0[0].im + cf0[1].im * si + cf0[2].im * si * si +
                     cf0[3].im * si * si * si;
    M2_tilde_re[i] = cf2[0].re + cf2[1].re * si + cf2[2].re * si * si +
                     cf2[3].re * si * si * si;
    M2_tilde_im[i] = cf2[0].im + cf2[1].im * si + cf2[2].im * si * si +
                     cf2[3].im * si * si * si;
  }
  for (i = N[0]; i < (N[0] + Nsin); i++) {
    si = sqrt(s[i] - METAP_M_MPION_SQUARED);
    M0_tilde_re[i] = cg0[0].re + cg0[1].re * si + cg0[2].re * si * si +
                     cg0[3].re * si * si * si;
    M0_tilde_im[i] = cg0[0].im + cg0[1].im * si + cg0[2].im * si * si +
                     cg0[3].im * si * si * si;
    M2_tilde_re[i] = cg2[0].re + cg2[1].re * si + cg2[2].re * si * si +
                     cg2[3].re * si * si * si;
    M2_tilde_im[i] = cg2[0].im + cg2[1].im * si + cg2[2].im * si * si +
                     cg2[3].im * si * si * si;
  }
...
void etapi_disc_finite_cv_plus(complex *M_etapi, complex_spline M_etapi_hat,
                               double s0, double L2, double *s, int N, int n) {
  int i, N_int;
  ...
  double f_re(double z, void *params) {
    ...
    double fct = (gsl_spline_eval(M_etapi_hat.re, z, acc) -
                  gsl_spline_eval(M_etapi_hat.re, x, acc)) /
                 (z * (z - x));
    return fct;
  }
  double f_im(double z, void *params) {
    double x = *(double *)params;
    double fct = (gsl_spline_eval(M_etapi_hat.im, z, acc) -
                  gsl_spline_eval(M_etapi_hat.im, x, acc)) /
                 (z * (z - x));
    return fct;
  }
  ...
  for (i = 0; i < N; i++) {
    ...
    M_etapi[i].re =
        temp.re + (-M_PI * M.im + M.re * log_1 + Mconst.re * log_2) / s[i];
    M_etapi[i].im =
        temp.im + (M_PI * M.re + M.im * log_1 + Mconst.im * log_2) / s[i];
...
void etapi_disc_finite_below_cut(complex *M_etapi, complex_spline M_etapi_hat,
                                 double s0, double L2, double s_min, double *s,
                                 int N, int n) {
  int i, N_int;
  ...
  for (i = 0; i < N; i++) {
    ...
    if (fabs(s[i]) < 1.e-5) {
      log_1 = 1. / L2 + s[i] / (2. * L2 * L2) +
              s[i] * s[i] / (3. * L2 * L2 * L2) +
              s[i] * s[i] * s[i] / (4. * L2 * L2 * L2 * L2) +
              s[i] * s[i] * s[i] * s[i] / (5. * L2 * L2 * L2 * L2 * L2);
    } else {
...
void etapi_disc_finite_complex(complex *M_etapi, complex_spline M_etapi_hat,
                               double s0, double L2, complex *s, int N, int n) {
  int i, N_int;
  ...
  double f_re(double z, void *params) {
    ...
    double fct = gsl_spline_eval(M_etapi_hat.re, z, acc) * (z - x.re) / denom -
                 gsl_spline_eval(M_etapi_hat.im, z, acc) * x.im / denom;
    return fct;
  }
  ...
  double f_im(double z, void *params) {
    complex x = *(complex *)params;
    double denom = z * (pow(z - x.re, 2.0) + x.im * x.im);
    double fct = gsl_spline_eval(M_etapi_hat.re, z, acc) * x.im / denom +
                 gsl_spline_eval(M_etapi_hat.im, z, acc) * (z - x.re) / denom;
    return fct;
  }
  ...
  for (i = 0; i < N; i++) {
    ...
    log_1.re =
        0.5 *
        (M_PI * fabs(s[i].im) + 2. * s[i].im * atan((s[i].re - L2) / s[i].im) -
         s[i].re *
             (log(s[i].im * s[i].im + pow(L2 - s[i].re, 2.)) - 2. * log(L2))) /
        (s[i].re * s[i].re + s[i].im * s[i].im);

    log_1.im = 0.5 *
               (M_PI * s[i].re * sign(s[i].im) +
                2. * s[i].re * atan((s[i].re - L2) / s[i].im) +
                s[i].im * (log(s[i].im * s[i].im + pow(L2 - s[i].re, 2.)) -
                           2. * log(L2))) /
               (s[i].re * s[i].re + s[i].im * s[i].im);
...
void build_etapi_inhomogeneity(complex **M0_inhom, complex **M1_inhom,
                               complex **M2_inhom, complex_spline M0_finite,
                               complex_spline M2_finite,
                               complex_spline *M0_sing, complex_spline *M2_sing,
                               complex *F0, complex *F2, complex *G0,
                               complex *G2, double *s_cv_plus,
                               double *s_cv_minus, double *s_below_cut,
                               complex *s_complex, double s_min, double L2,
                               int *N, int n0_sub, int n1_sub, int n2_sub) {
  ...
  etapi_disc_finite_below_cut(M0_etapi[3], M0_finite, s_etapi, L2, s_min,
                              s_below_cut, N[3], n0_sub);
  etapi_disc_finite_below_cut(M2_etapi[3], M2_finite, s_etapi, L2, s_min,
                              s_below_cut, N[3], n2_sub);

  inhomogenity_sqrt_below_cut(M0_etapi_hat[3], M0_sing, F0, G0, s_below_cut,
                              Q12_below_cut, s_etapi, L2, N[3], n0_sub);
  inhomogenity_sqrt_below_cut(M2_etapi_hat[3], M2_sing, F2, G2, s_below_cut,
                              Q12_below_cut, s_etapi, L2, N[3], n2_sub);

  etapi_disc_finite_below_cut(M0_etapi[0], M0_finite, s_etapi, L2, s_min,
                              s_cv_plus, i_etapi, n0_sub);
  etapi_disc_finite_below_cut(M2_etapi[0], M2_finite, s_etapi, L2, s_min,
                              s_cv_plus, i_etapi, n2_sub);

  inhomogenity_sqrt_below_cut(M0_etapi_hat[0], M0_sing, F0, G0, s_cv_plus,
                              Q12_cv_plus, s_etapi, L2, i_etapi, n0_sub);
  inhomogenity_sqrt_below_cut(M2_etapi_hat[0], M2_sing, F2, G2, s_cv_plus,
                              Q12_cv_plus, s_etapi, L2, i_etapi, n2_sub);

  etapi_disc_finite_below_cut(M0_etapi[1], M0_finite, s_etapi, L2, s_min,
                              s_cv_minus, N[1], n0_sub);
  etapi_disc_finite_below_cut(M2_etapi[1], M2_finite, s_etapi, L2, s_min,
                              s_cv_minus, N[1], n2_sub);

  inhomogenity_sqrt_below_cut(M0_etapi_hat[1], M0_sing, F0, G0, s_cv_minus,
                              Q12_cv_minus, s_etapi, L2, N[1], n0_sub);
  inhomogenity_sqrt_below_cut(M2_etapi_hat[1], M2_sing, F2, G2, s_cv_minus,
                              Q12_cv_minus, s_etapi, L2, N[1], n2_sub);

  etapi_disc_finite_cv_plus(M0_etapi[0] + i_etapi, M0_finite, s_etapi, L2,
                            s_cv_plus + i_etapi, N[0] - i_etapi, n0_sub);
  etapi_disc_finite_cv_plus(M2_etapi[0] + i_etapi, M2_finite, s_etapi, L2,
                            s_cv_plus + i_etapi, N[0] - i_etapi, n2_sub);

  inhomogenity_sqrt_cv_plus(M0_etapi_hat[0] + i_etapi, M0_sing, F0, G0,
                            s_cv_plus + i_etapi, R12_cv_plus + i_etapi,
                            Q12_cv_plus + i_etapi, s_etapi, L2, 1.,
                            N[0] - i_etapi, n0_sub);
  inhomogenity_sqrt_cv_plus(M2_etapi_hat[0] + i_etapi, M2_sing, F2, G2,
                            s_cv_plus + i_etapi, R12_cv_plus + i_etapi,
                            Q12_cv_plus + i_etapi, s_etapi, L2, 1.,
                            N[0] - i_etapi, n2_sub);

  etapi_disc_finite_complex(M0_etapi[2], M0_finite, s_etapi, L2, s_complex,
                            N[2], n0_sub);
  etapi_disc_finite_complex(M2_etapi[2], M2_finite, s_etapi, L2, s_complex,
                            N[2], n2_sub);

  inhomogenity_sqrt_complex(M0_etapi_hat[2], M0_sing, F0, G0, s_complex,
                            Q12_complex_f, Q12_complex_g, s_etapi, L2, N[2],
                            n0_sub);
  inhomogenity_sqrt_complex(M2_etapi_hat[2], M2_sing, F0, G0, s_complex,
                            Q12_complex_f, Q12_complex_g, s_etapi, L2, N[2],
                            n2_sub);
