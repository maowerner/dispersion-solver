...
double d2Omnes0(double (*delta)(double), delta_params Delta, double dO0) {
  ...
  double high_energy_reminder(double z, void *params) {
    double fct = Delta.a * (1. - z) /
                 (Delta.L2 * Delta.L2 * (Delta.b + pow(1. - z, -Delta.m)));
    return fct;
  }
...
complex Omnes_function(double (*delta)(double), delta_params Delta, complex s) {
  int M = 1500;
  complex temp;
  double A, B, C, D, r, phi, result, result1, result2, error, eps, eps1, eps2,
      s_temp;
  double pts[3];
  ...
  if (fabs(s.im) < eps) {
    ...
    double high_energy_reminder(double z, void *params) {
      double x = *(double *)params;
      double fct =
          Delta.a * pow(1. - z, Delta.m) /
          ((Delta.b * pow(1. - z, Delta.m) + 1.) * (Delta.L2 - x * (1. - z)));
      return fct;
    }

    double high_energy_reminder_cv(double z, void *params) {
      double y = *(double *)params;
      double fct = Delta.a * pow(1. - z, Delta.m) * (1. - y) /
                   ((Delta.b * pow(1. - z, Delta.m) + 1.) * Delta.L2);
      return fct;
    }
    ...
    if (s.re < Delta.s_th - eps) {
      ...
      gsl_integration_qags(&F, Delta.s_th, Delta.L2, eps1, eps2, M, w, &result,
                           &error);
      ...
      result = s.re / M_PI * (result - result1) +
               Delta.n * log(Delta.L2 / (Delta.L2 - s.re));
    }
      ...
      result1 = ((result1 - result2) * s.re +
                 (log(Delta.s_th / (s_temp - Delta.s_th)) +
                  log((Delta.L2 - s.re) / Delta.L2)) *
                     delta(s.re)) /
                    M_PI +
                Delta.n * log(Delta.L2 / (Delta.L2 - s.re));
      ...
      result = ((result - result1) * s.re +
                (log(Delta.s_th / (s.re - Delta.s_th)) +
                 log((Delta.L2 - s.re) / Delta.L2)) *
                    delta(s.re)) /
                   M_PI +
               Delta.n * log(Delta.L2 / (Delta.L2 - s.re));
      ...
      result = s.re / M_PI * (result - result1) +
               Delta.n * log(Delta.L2 / (s_temp - Delta.L2));

      F.function = &f2;
      s_temp = Delta.L2 - eps;
      F.params = &s_temp;

      gsl_integration_qawc(&F, Delta.s_th, Delta.L2, s.re, eps1, eps2, M, w,
                           &result1, &error);
      ...
      result1 = ((result1 - result2) * s.re +
                 (log(Delta.s_th / (s.re - Delta.s_th)) +
                  log((Delta.L2 - s_temp) / Delta.L2)) *
                     delta(s.re)) /
                    M_PI +
                Delta.n * log(Delta.L2 / (Delta.L2 - s_temp));
      ...
      gsl_integration_qawc(&F, 0., 1., s_temp, eps1, eps2, M, w, &result1,
                           &error);

      result = s.re / M_PI * (result - result1) +
               Delta.n * log(Delta.L2 / (s.re - Delta.L2));
    }
    ...
    double f3(double z, void *params) {
      ...
      double fct =
          delta(z) * (1.0 - x.re / z) / (pow(z - x.re, 2.0) + x.im * x.im);
      return fct;
    }
    ...
    double high_energy_cmp_re(double z, void *params) {
      complex x = *(complex *)params;
      double fct =
          Delta.a * pow(1. - z, Delta.m) * (Delta.L2 - x.re * (1. - z)) /
          ((Delta.b * pow(1. - z, Delta.m) + 1.) *
           (pow(Delta.L2 - x.re * (1. - z), 2.) + pow(x.im * (1. - z), 2.)));
      return fct;
    }
    ...
    double high_energy_cmp_im(double z, void *params) {
      complex x = *(complex *)params;
      double fct =
          Delta.a * pow(1. - z, Delta.m) * (x.im * (1. - z)) /
          ((Delta.b * pow(1. - z, Delta.m) + 1.) *
           (pow(Delta.L2 - x.re * (1. - z), 2.) + pow(x.im * (1. - z), 2.)));
      return fct;
    }
    ...
    gsl_integration_qags(&F, Delta.s_th, Delta.L2, eps1, eps2, M, w, &A,
                         &error);

    F.function = &f4;
    F.params = &s;

    gsl_integration_qags(&F, Delta.s_th, Delta.L2, eps1, eps2, M, w, &B,
                         &error);
...
void omnes_cv_plus(complex *omnes, double (*delta)(double), delta_params Delta,
                   double *s, int N) {
  int i;
  complex s_cmp;
  ...
  for (i = 0; i < N; i++) {
    ...
    if (isnan(omnes[i].re) == 1) {
      printf(
          "FATAL ERROR: In function omnes_cv_plus, omnes[%d].re=nan, s=%.10e\n",
          i, s[i]);
      exit(1);
    } else if (isnan(omnes[i].im) == 1) {
      printf(
          "FATAL ERROR: In function omnes_cv_plus, omnes[%d].im=nan, s=%.10e\n",
          i, s[i]);
      exit(1);
    }
  }
...
void omnes_cv_minus(complex *omnes, double (*delta)(double), delta_params Delta,
                    double *s, int N) {
  int i;
  complex s_cmp;
  ...
  for (i = 0; i < N; i++) {
    ...
    if (isnan(omnes[i].re) == 1) {
      printf("FATAL ERROR: In function omnes_cv_minus, omnes[%d].re=nan, "
             "s=%.10e\n",
             i, s[i]);
      exit(1);
    } else if (isnan(omnes[i].im) == 1) {
      printf("FATAL ERROR: In function omnes_cv_minus, omnes[%d].im=nan, "
             "s=%.10e\n",
             i, s[i]);
      exit(1);
    }
...
void omnes_complex(complex *omnes, double (*delta)(double), delta_params Delta,
                   complex *s, int N) {
  int i;
  ...
  for (i = 0; i < N; i++) {
    ...
    if (isnan(omnes[i].re) == 1) {
      printf("FATAL ERROR: In function omnes_complex, omnes[%d].re=nan, "
             "s.re=%.10e s.im=%.10e\n",
             i, s[i].re, s[i].im);
      exit(1);
    } else if (isnan(omnes[i].im) == 1) {
      printf("FATAL ERROR: In function omnes_complex, omnes[%d].im=nan, "
             "s.re=%.10e s.im=%.10e\n",
             i, s[i].re, s[i].im);
      exit(1);
    }
...
void omnes_below_cut(complex *omnes, double (*delta)(double),
                     delta_params Delta, double *s, int N) {
  int i;
  complex s_cmp;
  ...
  for (i = 0; i < N; i++) {
    ...
    if (isnan(omnes[i].re) == 1) {
      printf("FATAL ERROR: In function omnes_below_cut, omnes[%d].re=nan, "
             "s=%.10e\n",
             i, s[i]);
      exit(1);
    } else if (isnan(omnes[i].im) == 1) {
      printf("FATAL ERROR: In function omnes_below_cut, omnes[%d].im=nan, "
             "s=%.10e\n",
             i, s[i]);
      exit(1);
    }
...
void build_omnes(complex **omnes, double (*delta)(double), delta_params Delta,
