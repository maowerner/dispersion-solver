...
double integration_III_radius_phi(double phi) {
  ...
  if (z == -1.0) {
    ...
    eta = 1.0 / 3.0 *
          (acos(54.0 * pow(c - 1.0, 2.0) / pow(c + 3.0, 3.0) * pow(z, 2.0) -
                1.0));
...
complex Q12_real(double s, double x, double y) {
  ...
  gsl_integration_qagp(&F, pts, 3, abserr, relerr, w_size, w, &result.re,
                       &error);

  F.function = &Q12_im_int;
  gsl_integration_qagp(&F, pts, 3, abserr, relerr, w_size, w, &result.im,
                       &error);
