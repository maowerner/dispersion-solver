...
delta_params delta_high_energy_tale(delta_params delta) {
    ...
    Ddelta_L2 = (gsl_spline_eval(delta.spline,delta.L2+eps,delta.acc)-gsl_spline_eval(delta.spline,delta.L2-eps,delta.acc))/(2.*eps);
    delta.a = delta.m*pow(delta.n*M_PI-delta_L2,2.)/(delta.L2*Ddelta_L2);
...
double delta0(double s) {
    ...
    if (s<=delta0_params.s_th) {
        ...
        return delta0_params.n*M_PI-delta0_params.a/(delta0_params.b+pow(s/delta0_params.L2,delta0_params.m));
    }
    else if (pi0pi0==true) {
        if (s<=4.) {
            ...
            return atan((v0+2.*v2+3.*v0*v2*sigma)*sigma0/(3.+2.*v0*sigma+v2*sigma));
        }
        else {
            sigma = sqrt(1.-4./s);
            sigma0 = sqrt(1.-4.*pow(MPION0/MPION,2.)/s);
            ...
            if (s<x0) {
                ...
                return atan((2.*v2*(sigma0-sigma)+v0*(sigma0+sigma*(2.+3.*v2*v2*sigma*sigma0)))/(3.+v2*sigma*(2.*v0*(sigma-sigma0)+v2*(sigma+2.*sigma0))));
            }
            
            else if (s<x1) {
                eps = s/(x1-x0)+x0/(x0-x1);
                
                v0 = eps*tan(d0)/sigma+(1.-eps)*(0.2212+0.2895*(s-4.)/4.);
                v2 = eps*tan(d2)/sigma+(1.-eps)*(-0.0432-0.0803*(s-4.)/4.);
                ...
                return eps*gsl_spline_eval(delta0_params.spline,s,delta0_params.acc)+(1.-eps)*atan((2.*v2*(sigma0-sigma)+v0*(sigma0+sigma*(2.+3.*v2*v2*sigma*sigma0)))/(3.+v2*sigma*(2.*v0*(sigma-sigma0)+v2*(sigma+2.*sigma0))));
            }
        ...
        if (s<=X0) {
            ...
            return eps*gsl_spline_eval(delta0_params.spline,s,delta0_params.acc)+(1.-eps)*0.5*asin(4.*k/sqrt(s)*(0.218581+0.287486*k*k));
        }
...
double delta1(double s) {
    if (s<=delta1_params.s_th) {
        ...
        return delta1_params.n*M_PI-delta1_params.a/(delta1_params.b+pow(s/delta1_params.L2,delta1_params.m));
    }
    else {
...
//Gibt Phase der P-Welle (pipi-Streuung) in Abhängigkeit der Mandelstamvariable s wieder. Parametrisierung aus: https://arxiv.org/pdf/1102.2183.pdf
/*double delta1(double s) {
    s *= MPION*MPION;
    double Mk=493.7;
    double c=0;
    double f1,f2;
    double Mrho=773.6, Mp=139.6, B0=1.055, B1=0.15, s00=1050*1050; //Konstanten für f1
...
double delta2(double s) {
    ...
    //}
        ...
        return delta2_params.n*M_PI-delta2_params.a/(delta2_params.b+pow(s/delta2_params.L2,delta2_params.m));
    }
    else {
...
double delta_etapi(double s) {
    if (s<=delta_etapi_params.s_th) {
        ...
        return delta_etapi_params.n*M_PI-delta_etapi_params.a/(delta_etapi_params.b+pow(s/delta_etapi_params.L2,delta_etapi_params.m));
    }
    else if (pi0pi0==true) {
        if (s<4.*pow(MKAON/MPION,2.)) {
            double m,n,sp;
            
            m = (4.*pow(MKAON/MPION,2.)-pow(1.+META/MPION,2.))/(4.*pow(MKAON/MPION,2.)-pow(MPION0/MPION+META/MPION,2.));
            n = 4.*pow(MKAON/MPION,2.)*(1.-m);
            
            return gsl_spline_eval(delta_etapi_params.spline,m*s+n,delta_etapi_params.acc);
        }
        else {
            return gsl_spline_eval(delta_etapi_params.spline,s,delta_etapi_params.acc);
        }
    }
    else {
        return gsl_spline_eval(delta_etapi_params.spline,s,delta_etapi_params.acc);
