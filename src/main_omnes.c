...
int main(int argn, char **argc) {
  ...
  build_omnes(omnes0, abs_omnes0, delta0, s0, L2, 2. * M_PI, s_cv_plus,
              s_cv_minus, s_below_cut, s_complex, N_M_int);
  build_omnes(omnes1, abs_omnes1, delta1, s0, L2, M_PI, s_cv_plus, s_cv_minus,
              s_below_cut, s_complex, N_M_int);
  build_omnes(omnes2, abs_omnes2, delta2, s0, L2, 0., s_cv_plus, s_cv_minus,
              s_below_cut, s_complex, N_M_int);
  printf("Done.\n\n");
  ...
  for (i = 1; i < 21; i++) {
    ...
    build_inhomogenity_integrand(
        M0_hat, M1_hat, M2_hat, F0, G0, F1, G1, F2, G2, M0_avg, M1_avg, M2_avg,
        sin_delta0, sin_delta1, sin_delta2, abs_omnes0, abs_omnes1, abs_omnes2,
        s_cv_plus, s0, L2, N_M_avg, N_sin, n0_sub, n1_sub, n2_sub);
    printf("Done.\n\n");
  ...
  for (i = 0; i < N_M_int[2]; i++) {
    fprintf(fout, "%.3e %.3e %.3e %.3e %.3e %.3e %.3e\n", s_complex[i].re,
            s_complex[i].im, phi[i], Q32_complex_f[i].re, Q32_complex_f[i].im,
            Q32_complex_g[i].re, Q32_complex_g[i].im);
  }
  ...
  //        fprintf(fout,"%.20e %.20e %.20e %.20e %.20e %.20e %.20e %.20e %.20e
  //        %.20e %.20e %.20e
