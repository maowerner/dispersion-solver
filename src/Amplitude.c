...
void amplitude_cv_plus(complex_spline M, complex *omnes, complex *M_inhom, double *s, double s_th, double L2, int N, int n) {
    int i;
    double si,sf;
    complex m,O;
...
void amplitude_cv_minus(complex_spline M, complex *omnes, complex *M_inhom, double *s, double s_th, int N, int n) {
    int i;
    double si;
    complex m,O;
    double *M_re = (double *)malloc(N*sizeof(double));
...
void amplitude_below_cut(complex_spline M, complex *omnes, complex *M_inhom, double *s, double s_th, int N, int n) {
    int i;
    double sf;
    complex m,O;
    double *M_re = (double *)malloc(N*sizeof(double));
    ...
    for (i=0; i<N; i++) {
        ...
        if (isnan(m.re)==1) {
            printf("FATAL ERROR: In function amplitude_below_cut: m.re=nan, step=%d\n",i);
            exit(1);
        }
        else if (isnan(m.im)==1) {
            printf("FATAL ERROR: In function amplitude_below_cut: m.im=nan, step=%d\n",i);
            exit(1);
        }
        
        if (isnan(O.re)==1) {
            printf("FATAL ERROR: In function amplitude_below_cut: O.re=nan, step=%d\n",i);
            exit(1);
        }
        else if (isnan(O.im)==1) {
            printf("FATAL ERROR: In function amplitude_below_cut: O.im=nan, step=%d\n",i);
            exit(1);
        }
...
void amplitude_complex(complex_spline M, complex *omnes, complex *M_inhom, complex *s, double *y, char plusminus, int N, int n) {
    int i;
    double r,phi;
    complex m,O;
    double *M_re = (double *)malloc(N*sizeof(double));
...
void build_amplitude(complex_spline *M, complex **omnes, complex **M_inhom, double *s_cv_plus, double *s_cv_minus, double *s_below_cut, complex **s_cmp, double **y, double s0, double L2, int *N, int n) {
